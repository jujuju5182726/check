        ...
        try:
            async with session.get(url) as response:
                if response.status == 200:
                    ...
                    print("Data from API:", data)  # Print data from API
                    if 'result' in data:
                        balance = int(data['result']) / 10**18  # Convert wei to Ether
                        return balance
                    else:
                        print(f"No balance data found for {address}")
                        return None
                else:
...
async def run_with_asyncio():
    api_keys = ['79NE2HM482ZECC38MZNS365D1JNUGSMKFT',
                ...
                'YVJ5BSKNI7AUXIJ97EKKPFDYPGNB1NT7XI',
                'WTHIUS3XN1241H16PSHEYKAI56V1M1Q4JU',
                'FVQWCYZD5ST2NAKDVKDRC9IYEPV2ANBYI7',
